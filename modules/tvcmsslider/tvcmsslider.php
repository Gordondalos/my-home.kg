    ...
    protected function postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            if (Tools::strlen(Tools::getValue('url_' . $id_lang_default)) == 0) {
                $errors[] = $this->getTranslator()->trans('The URL is not set.', array(), 'Modules.TvcmsSlider.Admin');
            }
    ...
    protected function postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($languages as $language) {
                ...
                $imagedata = getimagesize($_FILES['image_'.$language['id_lang']]['tmp_name']);
                $width = $imagedata[0];
                $height = $imagedata[1];
    ...
    protected function getMultiLanguageInfoMsg()
    {
        return '<p class="alert alert-warning">'.
                    $this->getTranslator()->trans('Since multiple languages are activated on your shop, please mind'
                        .' to upload your image for each one of them', array(), 'Modules.TvcmsSlider.Admin').
                '</p>';
    }
    ...
    protected function getWarningMultishopHtml()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            $this->getTranslator()->trans('You cannot manage slides items from a "All Shops" or a "Group Shop" context,'
                .' select directly the shop you want to edit', array(), 'Modules.TvcmsSlider.Admin') .
            '</p>';
        } else {
            return '';
        }
    ...
    protected function getSharedSlideWarning()
    {
        return '<p class="alert alert-warning">'.
                    $this->trans('This slide is shared with other shops! All shops associated to this slide will apply'
                        .' modifications made here', array(), 'Modules.TvcmsSlider.Admin').
                '</p>';
    }
}

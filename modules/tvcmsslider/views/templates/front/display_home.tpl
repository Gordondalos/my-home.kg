...
* @author PrestaShop SA <contact@prestashop.com>
    ...
    *}
    ...
    {/if}
    <div class="tvcms-slider-offerbanner-wrapper container-fluid wow fadeInUp">
        <div class="tvcmsmain-slider-wrapper" data-speed='{$main_slider_js.speed}' data-pause-hover='{$main_slider_js.pause}'>
            <div class='tvcms-main-slider'>
                <div class='tv-main-slider'>
                    <div id='tvmain-slider' class="owl-theme owl-carousel">
                        ...
                        <a href="{$slide['url']}">
                            {/if}
                            <img class="tvmain-slider-img" src='{$slide["image_url"]}' alt='{l s="Main Slider" mod="tvcmsslider"}' {* title='#tvmain-slider-img-{$i}' *} data-caption-id='tvmain-slider-img-{$i}' width="{$slide['width']}" height="{$slide['height']}" {if $i!=1}style="display:none" {/if}>
                            {if ($slide['title'] || $slide['description']) && {$slide["class_name"]}!="tvmain-slider-contant-none" }
                            <div id='tvmain-slider-img-{$i}' class="nivo-html-caption caption tvmain-slider-content-inner {if $i == 1}active{/if} {$slide['class_name']}" {if $i!=1} style="display:none" {/if}>
                                <div class='tvmain-slider-contant'>
                                    ...
                                    <div class="tvmain-slider-btn">
                                        <div class="btn tvall-inner-btn tvmain-slider-button animated"><span>{$slide['btn_caption']}</span></div>
                                    </div>
                                    {/if}
                                </div>
                            </div>

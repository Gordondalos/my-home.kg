...
* @copyright 2007-2021 Prestashop
...
<div class="form-group {if $ver == 1}col-md-12{/if}">
    ...
    <div class="form-group" id="url_video" style=" {if empty($total)} display: block; {else}  {if empty($embed)} display: none; {else} display: block; checked {/if}{/if}">
        ...
        <div class="translatable-field1 lang1-{$value['id_lang']|escape:'htmlall':'UTF-8'}" style="{if $value['checked']==1}display: block;{/if}{if $value['checked']!=1}display: none;{/if}">
                ...
                {if empty($embed)}
                <textarea name="name_url[{$value['id_lang']|escape:'htmlall':'UTF-8'}]" {if $ver == 1}style="width: -webkit-fill-available;"{/if}  id="note" cols="50" value="<iframe width='700' height='315' 
    src='https://www.youtube.com/embed/PUvWR0HdQmg' frameborder='0' 
    allow='autoplay; encrypted-media' allowfullscreen></iframe>" rows="10"></textarea>
                {/if}
                {if !empty($embed)}
                    <textarea value="<iframe width='700' height='315' 
    src='https://www.youtube.com/embed/PUvWR0HdQmg' frameborder='0' 
    allow='autoplay; encrypted-media' allowfullscreen></iframe>" name="name_url[{$value['id_lang']|escape:'htmlall':'UTF-8'}]" {if $ver == 1}style="width: -webkit-fill-available;"{/if}  id="note" cols="50" rows="10">{foreach $embed key =key1 item=value1}{if $value1['id_lang'] == $value['id_lang']}{$value1['text_url']|escape:'htmlall':'UTF-8'}{/if}{/foreach}</textarea>
                {/if}

...
class TvcmsVerticalMenu extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        parent::__construct();
        $this->displayName = $this->l('HRXTemplate - Vertical Menu');
    ...
    protected function postProcess()
    {
        ...
        if (Tools::isSubmit('submitMenuItem')) {
            ...
            foreach ($languages as $language) {
                ...
                $menu_item->style[$language['id_lang']] = Tools::getValue('style_'.$language['id_lang']);
            }
            if (!$errors) {
                if (!Tools::getValue('id_tvcmsverticalmenu')) {
                    if (!$menu_item->add()) {
    ...
    public function hookdisplayNavFullWidth()
    {
        if (!Cache::isStored('showVerticalMenuData')) {
            $output = $this->showVerticalMenuData(false);
    ...
    {
        if (!Cache::isStored('showVerticalMenuDataLeft')) {
            $output = $this->showVerticalMenuDataLeft(false);
            Cache::store('showVerticalMenuDataLeft', $output);
        }
        return Cache::retrieve('showVerticalMenuDataLeft');
    }
    ...
    public function hookdisplayVerticalMenu()
    {
        if (!Cache::isStored('showVerticalMenuData')) {
            $output = $this->showVerticalMenuData(false);
            Cache::store('showVerticalMenuData', $output);
    ...
    public function showVerticalMenuDataLeft($isAjax = true)
    {
        $id_lang = (int)$this->context->language->id;
        ...
        return $this->display(__FILE__, 'views/templates/hook/tvcmsverticalmenu-data-left.tpl');
    }
    ...
    public function getRespCategories(
        ...
        if (!empty($is_intersected)) {
            ...
            if ($sub_title != '') {
                ...
                $this->respMenu .= '<span class="tvmenu-subtitle" '.$style.'>'.$sub_title.'</span>';
            }
    ...
    public function hookActionShopDataDuplication($params)
    {
        ...
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'tvcmsverticalmenu_lang (`id_tvcmsverticalmenu`, `id_shop`,
                `id_lang`, `title`, `link`, `subtitle`,`style`)
            SELECT id_tvcmsverticalmenu, '.(int)$params['new_id_shop'].', id_lang, title, link, subtitle,style
    ...
    public function hookActionObjectLanguageAddAfter($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'tvcmsverticalmenu_lang (`id_tvcmsverticalmenu`, `id_shop`, `id_lang`,
                `title`, `link`, `subtitle`,`style`)
            SELECT id_tvcmsverticalmenu, id_shop, '.(int)$params['object']->id.', title, link, subtitle, style
            FROM '._DB_PREFIX_.'tvcmsverticalmenu_lang

...
<div class="panel"><h3><i class="icon-list-ul"></i> {l s='Menu Item' mod='tvcmsverticalmenu'}</h3>
	<div class="form-wrapper" id="menuContent" >
		...
		<div class="form-group">
			<label class="control-label col-lg-3">{l s='Style' mod='tvcmsverticalmenu'}</label>
			<div class="col-lg-9">
				{foreach from=$languages item=language}
					{if $languages|count > 1}
						<div class="translatable-field lang-{$language.id_lang|intval}" {if $language.id_lang != $id_language}style="display:none"{/if}>
					{/if}
					<div class="col-lg-10">
					<input type="text" class="style" id="style_{$language.id_lang|intval}" name="style_{$language.id_lang|intval}" value="{if $menu->style[$language.id_lang|intval]}{$menu->style[$language.id_lang|intval]}{/if}"/>
					<p>{l s='You have to pur inline css. example:- (color:red;font-size:12px)' mod='tvcmsverticalmenu'}</p>
						<p>{l s='This css is only use for sub title. if you do not want then keep it blank.' mod='tvcmsverticalmenu'}</p>
					</div>
					{if $languages|count > 1}
						<div class="col-lg-2">
							<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">

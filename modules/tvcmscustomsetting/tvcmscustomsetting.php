...
class TvcmsCustomSetting extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('HRXTemplate - Custom Setting');
        $this->description = $this->l('It is use of Custom Setting in HRXTemplate Theme');
    ...
    {
        if (!(int)Tab::getIdFromClassName('AdminHRXTemplate')) {
            $parent_tab = new Tab();
            // Need a foreach for the language
            foreach (Language::getLanguages() as $language) {
                $parent_tab->name[$language['id_lang']] = $this->l('HRXTemplate Extension');
            }
            $parent_tab->class_name = 'AdminHRXTemplate';
        ...
        foreach (Language::getLanguages() as $language) {
            ...
            $tab->id_parent = (int)Tab::getIdFromClassName('AdminHRXTemplate');
    ...
    public function createVariable()
    {
        ...
        foreach ($languages as $lang) {
            ...
            $result['TVCMSCUSTOMSETTING_DOWNLOAD_APPS_SUB_TITLE'][$lang['id_lang']] = 'Fast, Simple & De'
            .'lightful. All It takes is 30 Seconds to Download.';
            $result['TVCMSCUSTOMSETTING_DOWNLOAD_APPS_DESC'][$lang['id_lang']] = 'Fast, Simple & Delightful. All It '
            ...
            $result['TVCMSCUSTOMSETTING_NEWSLETTER_SHORT_DESC'][$lang['id_lang']] = 'Sign up for our newletter '
            ...
            $tmp = 'India\'s Fastest Online Shopping Destination';
        ...
        Configuration::updateValue('TVCMSCUSTOMSETTING_FOOTER_IMAGE', 'demo_img_1.jpg');
    ...
    public function getContent()
    {
        ...
        $output = $message
                    .'<div class="tvcmsadmincustom-setting">'
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitTvcmsThemeOptionForm')) {
            if (!empty($_FILES['tvcmscustomsetting_custom_pattern']['name'])) {// Bg color and pattern
            ...
            if (!empty($_FILES['tvcmscustomsetting_custom_body_pattern']['name'])) {// Body color and pattern
        ...
        if (Tools::isSubmit('submitTvcmsAppLinkForm')) {
        ...
        if (Tools::isSubmit('submitTvcmsCustomTitleForm')) {
    ...
    public function makeInslineStyleSheet()
    {
        ...
        if (Configuration::get('TVCMSCUSTOMSETTING_THEME_OPTION') == 'theme_custom') {
            ...
            $themeCssPath = '/'.Configuration::get('TVCMSCUSTOMSETTING_THEME_OPTION')
    ...
    protected function renderForm()
    {
        ...
        return $helper->generateForm(array(
            $this->tvcmsThemeOptionForm(),
            $this->tvcmsAppLinkForm(),
            $this->tvcmsFooterProductForm(),
        ));
    }
    ...
    public function hookdisplayTopOfferText()
    {
        ...
        if (!Cache::isStored('tvcmscustomsetting_displaytopoffertext')) {
            if (Configuration::get('TVCMSCUSTOMSETTING_CUSTOM_TEXT_STATUS')) {
                $html .= '<div class="tvheader-nav-offer-text"><i class=\'material-icons\'>local_offer</i>'
                .Configuration::get('TVCMSCUSTOMSETTING_CUSTOM_TEXT', $id_lang, true).'</div>';
            }
            $output = $html;
    ...
    public function hookdisplayCopyRightText()
    {
        ...
        if (!Cache::isStored('tvcmscustomsetting_display_copy_right_text.tpl')) {
            if (Configuration::get('TVCMSCUSTOMSETTING_COPY_RIGHT_TEXT_STATUS')
                && Configuration::get('TVCMSCUSTOMSETTING_COPY_RIGHT_TEXT', $id_lang)) {
    ...
    public function hookdisplayHeader()
    {
        ...
        $this->context->controller->addJS($this->_path.'views/js/wow.js');
        ...
        $this->context->controller->addCSS($this->_path.'views/css/animation.css');

        $tvcms_setting = true;
        Media::addJsDef(array('tvcms_setting' => $tvcms_setting));

        $paths = _MODULE_DIR_.$this->name."/views/img/";
        $this->context->smarty->assign('paths', $paths);

        $tmp = Configuration::get('TVCMSCUSTOMSETTING_WOW_JS');
        Media::addJsDef(array('TVCMSCUSTOMSETTING_WOW_JS' => $tmp));

        $tmp = Configuration::get('TVCMSCUSTOMSETTING_HOVER_IMG');
        Media::addJsDef(array('TVCMSCUSTOMSETTING_HOVER_IMG' => $tmp));

        $tmp = Configuration::get('TVCMSCUSTOMSETTING_MAIN_MENU_STICKY');
        Media::addJsDef(array('TVCMSCUSTOMSETTING_MAIN_MENU_STICKY' => $tmp));
        ...
        $tmp = Configuration::get('TVCMSCUSTOMSETTING_BOTTOM_OPTION');
        Media::addJsDef(array('TVCMSCUSTOMSETTING_BOTTOM_OPTION' => $tmp));

        $tmp = Configuration::get('TVCMSCUSTOMSETTING_PRODUCT_PAGE_BOTTOM_STICKY_STATUS');
        Media::addJsDef(array('TVCMSCUSTOMSETTING_PRODUCT_PAGE_BOTTOM_STICKY_STATUS' => $tmp));
        
        $tmp = Configuration::get('TVCMSFRONTSIDE_THEME_SETTING_SHOW');
        Media::addJsDef(array('TVCMSFRONTSIDE_THEME_SETTING_SHOW' => $tmp));

        $tmp = Configuration::get('TVCMSCUSTOMSETTING_VERTICAL_MENU_OPEN');
        Media::addJsDef(array('TVCMSCUSTOMSETTING_VERTICAL_MENU_OPEN' => $tmp));
